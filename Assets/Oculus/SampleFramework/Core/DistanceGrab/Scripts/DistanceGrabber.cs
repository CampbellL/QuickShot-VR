...
namespace OculusSampleFramework
{
    ...
    public class DistanceGrabber : OVRGrabber
    {
        ...
        [HideInInspector] public bool useHand = true; //Custom
        ...
        public override void GrabBegin(OVRGrabbable obj = null /*Custom*/)
        {
            DistanceGrabbable closestGrabbable = (obj) ? (DistanceGrabbable)obj : m_target; //Custom
            Collider closestGrabbableCollider = (obj && closestGrabbable && closestGrabbable.grabPoints.Length > 0) ?  //Custom
                                                                                    closestGrabbable.grabPoints[0] : 
                                                                                    m_targetCollider;
        ...
        protected override void MoveGrabbedObject(Vector3 pos, Quaternion rot, bool forceTeleport = false)
        {
            ...
            //Custom begin (two handed grab system --> primary hand looks at secondary hand)
            var weaponScript = grabbedObject.GetComponent<WeaponBehaviour>();
            Quaternion customRot = Quaternion.identity;
            bool useCustomRotation = false;

            if (weaponScript && weaponScript.isTwoHanded)
            {
                useCustomRotation = true;
                customRot = grabbedObject.GetComponent<WeaponBehaviour>().GetLookAtSecondaryHandRotation();

                var eulerAng = rot.eulerAngles;
                eulerAng.x = 0;
                eulerAng.y = 0;
                Quaternion newRot = Quaternion.Euler(eulerAng);
                
                customRot *= (newRot * m_grabbedObjectRotOff);
            }
            //Custom end 
            ...
            if (m_movingObjectToHand)
            {
                ...
                if(travel * travel * 1.1f > dir.sqrMagnitude)
                {
                    ...
                    {
                        //parent the grabbed object to the custom hand when it arrives. this prevents jittering
                        m_grabbedObj.transform.SetParent(transform);
                        grabbedRigidbody.transform.localRotation = Quaternion.Euler(0, 0,
                            (m_controller == OVRInput.Controller.RTouch) ? 90 : -90);
            ...
            //grabbedRigidbody.MoveRotation((useCustomRotation && !m_movingObjectToHand) ? customRot : grabbableRotation); //Custom old version
            if (useCustomRotation) grabbedObject.transform.rotation = customRot; //Custom
        }
        ...
        protected bool FindTarget(out DistanceGrabbable dgOut, out Collider collOut)
        {
            ...
            foreach (OVRGrabbable cg in m_grabCandidates.Keys)
            {
                ...
                bool canGrab = grabbable != null && grabbable.InRange && !(grabbable.isGrabbed && !grabbable.allowOffhandGrab); //&& grabbable.activated; //Custom
