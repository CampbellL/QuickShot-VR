...
public class OVRGrabber : MonoBehaviour
{
    ...
    public GameObject startWeapon; //if allowWeaponPickup == false
    [HideInInspector] public bool isUsingRifle;
    private OVRGrabbable _startWeapon;
    [Space]
    ...
    protected virtual void Start()
    {
        ...
        //Custom
        if (!allowWeaponPickup)
        {
            if (startWeapon)
            {
                startWeapon.SetActive(true);
                GrabBegin(startWeapon.GetComponent<OVRGrabbable>());
            }
            else
            {
                Debug.LogWarning("No start weapon selected! Setting allowWeaponPickup to true!");
    ...
    protected void CheckForGrabOrRelease(float prevFlex)
    {
        ...
        else if ((m_prevFlex <= grabEnd) && (prevFlex > grabEnd) && !isUsingRifle && allowWeaponPickup) //Custom
        {
            GrabEnd();
        }
    }

    public virtual void GrabBegin(OVRGrabbable obj = null)
    {
    ...
    public void GrabEnd(bool isRifle = false)
    {
        if (m_grabbedObj != null)
        {
            ...
            GrabbableRelease(linearVelocity, angularVelocity, isRifle);
        }
    ...
    protected void GrabbableRelease(Vector3 linearVelocity, Vector3 angularVelocity, bool isRifle = false)
    {
        ...
        if(!isRifle) {
            if(_startWeapon != null)
                Destroy(m_grabbedObj); //destroy grabbed object if release is not allowed
            else
                m_grabbedObj.transform.parent = null; //Custom (only deparent if the object is not the rifle)
        }
    ...
    {
        if(_startWeapon != null) Destroy(m_grabbedObj);
    }
}
